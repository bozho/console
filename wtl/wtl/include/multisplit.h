 ...
 *																			*
 * Written by Ted Szoczei (ted.szoczei@nimajin.com),						*
 ...
 *																			*
 * Find the latest version at http://www.nimajin.com						*
 *																			*
 ****************************************************************************
 * REVISIONS																*
 * ---------																*
 * DATE       VER   CHANGES /BY												*
 * ---------- ----- -----------												*
 * 08-04-25: Added PanesCount property and Contains method.					*
 *			 Revised Insert so index >= PanesCount will Add, not assert./tsz*
 * 04-12-13: Added mouse wheel scrolling support.							*
 *			 Added SetFocus to MouseActivate so mouse wheel works whenever	*
 *			 window active. /tsz											*
 * 03-05-16: Initial release. /tsz											*
 *																			*
 ****************************************************************************
 * NOTES																	*
 * -----																	*
 * Limited mode behaviour:													*
 *	For one or two panes, multi-splitter acts the same as a regular two-pane*
 *	 splitter.																*
 *	Panes are limited in size to fill visible area.							*
 *	The last pane resizes to fit the area left by other panes.				*
 *	Resizing a pane affects only the pane on the other side of the splitter *
 *	bar.																	*
 *	Specifying -1 for a pane size resizes all panes to fit evenly within the*
 *	window.																	*
 *																			*
 * Scroll mode behaviour:													*
 *  Panes always start with the size specified, the virtual display area	*
 *	grows to fit them.														*
 *	Panes can be any size.													*
 *	Resizing a pane moves all following panes.								*
 *	There is always an empty square pane at the end for the last pane to	*
 *	resize into.															*
 *	Specifying -1 for a pane size gives it the average size of all existing *
 *	panes, or evenly divides them into visible space if all are -1.			*
 *																			*
 * All extended styles can be switched at any time by calling				*
 *	SetExtendedStyle.														*
 * Panes can be added to the end or inserted between existing panes.		*
 * Panes can be removed or replaced. Panes are indexed starting with 0.		*
 *																			*
 * No right/bottom alignment support.										*
 *																			*
 ****************************************************************************/
